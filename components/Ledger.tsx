  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-1">
        <button onClick={onPrevMonth} className="text-[var(--muted)] hover:text-[var(--text)] text-lg px-2 transition-colors">‹</button>
        <button onClick={onToday} className="text-lg font-semibold text-[var(--text)] tracking-tight">{monthLabel}</button>
        <button onClick={onNextMonth} className="text-[var(--muted)] hover:text-[var(--text)] text-lg px-2 transition-colors">›</button>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-1">
        <span
          ...
          className={`text-[14px] px-2.5 py-1.5 rounded-full border cursor-pointer ${filter.indulgence ? 'border-[var(--accent)] bg-[var(--accent)]/15 text-[var(--accent)]' : 'border-[var(--border)]/70 bg-[var(--bg)]/50 text-[var(--muted)] hover:bg-[var(--bg)]/80 hover:border-[var(--accent)]/60 transition-colors'}`}
        >indulgence</span>
        <span
          onClick={() => setFilter({ ...filter, essential: !filter.essential })}
          className={`text-[14px] px-2.5 py-1.5 rounded-full border cursor-pointer ${filter.essential ? 'border-[var(--accent)] bg-[var(--accent)]/15 text-[var(--accent)]' : 'border-[var(--border)]/70 bg-[var(--bg)]/50 text-[var(--muted)] hover:bg-[var(--bg)]/80 hover:border-[var(--accent)]/60 transition-colors'}`}
        >essential</span>
        <span
          onClick={() => setFilter({ ...filter, regret: !filter.regret })}
          className={`text-[14px] px-2.5 py-1.5 rounded-full border cursor-pointer ${filter.regret ? 'border-[var(--accent)] bg-[var(--accent)]/15 text-[var(--accent)]' : 'border-[var(--border)]/70 bg-[var(--bg)]/50 text-[var(--muted)] hover:bg-[var(--bg)]/80 hover:border-[var(--accent)]/60 transition-colors'}`}
        >regret</span>
        {anyChipActive && (
          <span onClick={() => setFilter(emptyFilter())} className="text-[14px] px-2.5 py-1.5 text-[var(--muted)] underline cursor-pointer">
            clear
          </span>
        )}
      </div>
      <div className="flex flex-wrap gap-1.5 mb-3">
        {categories.map((c) => (
          <span
            ...
            className={`text-[14px] px-2.5 py-1.5 rounded-full border cursor-pointer ${filter.categories.has(c) ? 'border-[var(--accent)] bg-[var(--accent)]/15 text-[var(--accent)]' : 'border-[var(--border)]/70 bg-[var(--bg)]/50 text-[var(--muted)] hover:bg-[var(--bg)]/80 hover:border-[var(--accent)]/60 transition-colors'}`}
          >{c}</span>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-auto -mx-1 px-1.5">
        {grouped.length === 0 && (
          <div className="text-center mt-10">
            <p className="text-base text-[var(--muted)]">No transactions match</p>
          </div>
        )}
        {grouped.map(([date, txns]) => {
          ...
          return (
            <div key={date}>
              <div className="sticky top-0 bg-[var(--surface)] flex justify-between text-[14px] text-[var(--muted)] uppercase tracking-wide font-semibold py-2">
              ...
              {txns.map((t) => (
                ...
                >
                  ...
                  <span className="text-base text-[var(--text)] truncate flex-1">{t.description}</span>
                  <span
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-[var(--muted)] hover:text-[var(--danger)] text-base shrink-0 transition-opacity"
                    aria-label="Delete"
                  >✕</button>
                </div>
              ))}
