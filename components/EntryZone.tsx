  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4" style={{ background: 'color-mix(in srgb, var(--bg), transparent 40%)' }} onClick={onCancelEdit}>
      ...
      >
        ...
        <p className="text-sc-12 text-[var(--muted)] mb-1.5">Category</p>
        ...
        <div className="flex gap-4 mb-2 text-sc-13 text-[var(--muted)]">
        ...
        <p className="text-sc-12 text-[var(--muted)] mb-1.5">Emotional tag</p>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {TAGS.map((t) => (
        ...
        <div className="flex gap-2 mb-2">
          <input
            ...
            className="flex-1 bg-[var(--bg)]/50 border border-[var(--border)]/70 rounded-lg px-2 py-1.5 text-sc-14 text-[var(--text)]"
          />
          <select
            value={detail.repeats}
            onChange={(e) => setDetail({ ...detail, repeats: e.target.value as NewTransaction['repeats'] })}
            className="flex-1 bg-[var(--bg)]/50 border border-[var(--border)]/70 rounded-lg px-2 py-1.5 text-sc-14 text-[var(--muted)]"
          >
        ...
        <input
          ...
          className="w-full bg-[var(--bg)]/50 border border-[var(--border)]/70 rounded-lg px-3 py-1.5 text-sc-14 text-[var(--text)] mb-3"
        />

        <div className="flex gap-2 pt-3 border-t border-[var(--border)]/30">
          <button type="submit" className="flex-1 bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-14 font-medium">Save changes</button>
          <button
            type="button"
            onClick={async () => { await onDelete(editingTxn.id); onCancelEdit(); }}
            className="border border-[var(--danger)] text-[var(--danger)] rounded-lg px-3 text-sc-14"
          >
            Delete
          </button>
          <button type="button" onClick={onCancelEdit} className="bg-[var(--bg)]/50 border border-[var(--border)]/70 text-[var(--muted)] rounded-lg px-3 text-sc-14">
            Cancel
          </button>
        </div>
      </form>
