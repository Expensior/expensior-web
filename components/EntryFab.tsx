...
const chip = (active: boolean) =>
  `text-sc-17 font-medium px-3 py-2 rounded-lg cursor-pointer transition-colors ${
              ...
              {recurringOpen && (
                <div className="border-t border-[var(--border)]/30 mt-4 pt-4 flex flex-col gap-2">
                  {recurringTemplates.length === 0 && (
                    <p className="text-sc-20 text-[var(--muted)]">No templates yet — add one in Settings.</p>
                  )}
                  {recurringTemplates.map((t) => (
                    <div key={t.id} className="flex justify-between items-center bg-[var(--accent)]/8 rounded-lg px-4 py-3">
                      <div>
                        <p className="text-sc-20 font-medium text-[var(--text)]">{t.name}</p>
                        <p className="text-sc-18 text-[var(--muted)]">{fmt(t.amount)} · {t.cadence}</p>
                      </div>
                      <button
                        onClick={async () => { await onLogRecurring(t); flashToast(`Logged ${t.name}`); closeAll(); }}
                        className="bg-[var(--accent)] text-[var(--bg)] text-sc-18 font-medium px-4 py-2 rounded-md"
                      >
                        Log now
                      </button>
                    </div>
...
function Tile({ icon: Icon, label, primary, locked, onClick }: { icon: any; label: string; primary?: boolean; locked?: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-2" style={locked ? { opacity: 0.45 } : undefined}>
      ...
      <p className="text-sc-16 text-[var(--text)]">{label}</p>
    </button>
  );
}
...
function RecurringTile({ open, onClick }: { open: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-2">
      ...
      <p className="text-sc-16 text-[var(--text)]">Recurring</p>
    </button>
  );
}
...
function BackHeader({ label, onBack }: { label: string; onBack: () => void }) {
  return (
    <div className="flex items-center gap-3 px-6 pt-6 pb-2">
      ...
      <p className="text-sc-20 font-medium text-[var(--text)]">{label}</p>
    </div>
  );
}
    ...
    <div className="p-5">
      ...
      {mode === 'quick' && (
        <>
          <input
            ...
            className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-3 py-2.5 text-sc-20 text-[var(--text)] placeholder:text-[var(--muted)] mb-2"
          />
          {preview && (
            <div className="bg-[var(--bg)]/30 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <input type="date" value={preview.date} onChange={(e) => setPreview({ ...preview, date: e.target.value })} className="ml-auto bg-[var(--surface)] border border-[var(--border)]/40 rounded px-1.5 py-1 text-sc-15 text-[var(--muted)]" />
              </div>
              <button onClick={confirmQuick} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium">Add transaction</button>
            </div>
          )}
        </>
      )}
      ...
      {mode === 'detailed' && (
        <form onSubmit={confirmDetailed}>
          ...
          <input type="date" value={detail.date} onChange={(e) => setDetail({ ...detail, date: e.target.value })} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-sc-18 text-[var(--text)] mb-2" />
          <button type="submit" className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium">Add transaction</button>
        </form>
      )}
    </div>
  );
...
function VoiceView({ categories, hasApiKey, onBack, onAdd, onDone }: { categories: string[]; hasApiKey: boolean; onBack: () => void; onAdd: (t: NewTransaction) => Promise<void>; onDone: (msg: string) => void }) {
    ...
    rec.onend = async () => {
      ...
      if (finalText.trim()) {
        ...
        if (amount) {
          ...
          const guess = await smartCategorize(supabase, description, categories, hasApiKey);
          setPreview({ amount, description, category: guess.category, indulgence: guess.indulgence, date });
  ...
  return (
    <div className="p-5">
      ...
      {unsupported && (
        <p className="text-sc-18 text-[var(--muted)] mt-2">Voice input isn&apos;t supported in this browser — it works in Chrome and Edge, not Firefox or Safari on some platforms.</p>
      )}
      {!unsupported && !preview && !categorizing && (
        <div className="text-center py-3">
          ...
          <p className="text-sc-18 text-[var(--muted)]">{listening ? (transcript || 'Listening…') : 'Tap to speak'}</p>
        </div>
      )}
      {categorizing && <p className="text-sc-18 text-[var(--muted)] text-center py-3">Categorizing…</p>}
      {preview && (
...
function SmsView({ categories, hasApiKey, onBack, onBulkAdd, onDone }: { categories: string[]; hasApiKey: boolean; onBack: () => void; onBulkAdd: (items: NewTransaction[]) => Promise<void>; onDone: (msg: string) => void }) {
  const [text, setText] = useState('');
  ...
  async function toReviewable(found: { amount: number; description: string; date: string }[]) {
    setCategorizing(true);
    const supabase = createClient();
    const withCategories = await Promise.all(
      found.map(async (c) => {
        const guess = await smartCategorize(supabase, c.description, categories, hasApiKey);
        return { ...c, category: guess.category, indulgence: guess.indulgence, selected: true };
      })
    );
    setCandidates(withCategories);
    setCategorizing(false);
  }

  async function parse() {
    await toReviewable(parseBulkText(text));
  }
  ...
  return (
    <div className="p-5">
      ...
      {!candidates && !csvData && !categorizing && (
        <>
          <textarea
            value={text}
            ...
            rows={4}
            className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-3 py-2 text-sc-18 text-[var(--text)] placeholder:text-[var(--muted)] mb-2"
          />
          <button onClick={parse} disabled={!text.trim()} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium disabled:opacity-50 mb-3">Parse thread</button>

          <div className="flex items-center gap-2 mb-3">
            <div className="flex-1 h-px bg-[var(--border)]/30" />
            <span className="text-sc-15 text-[var(--muted)]">or</span>
            <div className="flex-1 h-px bg-[var(--border)]/30" />
          </div>
          ...
          <button onClick={() => csvFileRef.current?.click()} className="w-full flex items-center justify-center gap-2 border border-dashed border-[var(--accent)]/40 rounded-lg py-2.5 text-sc-18 font-medium text-[var(--text)]">
            <IconFileSpreadsheet size={20} className="text-[var(--accent)]" />
            Upload a bank or UPI statement (CSV)
          </button>
          {csvError && <p className="text-sc-17 text-[var(--danger)] mt-2">{csvError}</p>}
        </>
      )}

      {categorizing && <p className="text-sc-18 text-[var(--muted)] text-center py-4">Categorizing…</p>}

      {csvData && !candidates && (
        <div>
          <p className="text-sc-18 text-[var(--text)] font-medium mb-1">Couldn&apos;t auto-detect columns</p>
          <p className="text-sc-17 text-[var(--muted)] mb-3">Match your statement&apos;s columns once — Expensior will recognize this format going forward.</p>

          <label className="text-sc-15 text-[var(--muted)] block mb-1">Date column</label>
          <select value={mapDate} onChange={(e) => setMapDate(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-sc-18 text-[var(--text)] mb-2">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <label className="text-sc-15 text-[var(--muted)] block mb-1">Description column</label>
          <select value={mapDesc} onChange={(e) => setMapDesc(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-sc-18 text-[var(--text)] mb-2">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <label className="text-sc-15 text-[var(--muted)] block mb-1">Amount column (debit or signed amount)</label>
          <select value={mapAmount} onChange={(e) => setMapAmount(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-sc-18 text-[var(--text)] mb-3">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <div className="flex gap-2">
            <button onClick={confirmMapping} className="flex-1 bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium">Continue</button>
            <button onClick={() => setCsvData(null)} className="border border-[var(--border)]/70 text-[var(--muted)] rounded-lg px-3 text-sc-18">Cancel</button>
          </div>
        </div>
      )}
      ...
      {candidates && (
        <>
          ...
          {candidates.length > 0 && (
            <button onClick={confirm} disabled={selectedCount === 0} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium disabled:opacity-50">
    ...
    return (
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 mb-2 mt-1">
          ...
          <p className="text-sc-18 font-medium text-[var(--text)]">Needs a Claude API key</p>
        </div>
        <p className="text-sc-17 text-[var(--muted)] leading-relaxed">
...
function GmailView({
  connected, categories, hasApiKey, onBack, onBulkAdd, onDone,
}: {
  connected: boolean;
  categories: string[];
  hasApiKey: boolean;
    ...
    return (
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 mb-2 mt-1">
          ...
          <p className="text-sc-18 font-medium text-[var(--text)]">Not connected yet</p>
        </div>
        <p className="text-sc-17 text-[var(--muted)] leading-relaxed mb-3">
          Read-only access — this app can search for receipts and order confirmations, never send, delete, or modify anything.
        </p>
        <button onClick={connect} disabled={connecting} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-sc-18 font-medium disabled:opacity-60">
  ...
  return (
    <div className="p-5">
      ...
      {!candidates && (
        <div className="mt-1">
          <p className="text-sc-17 text-[var(--muted)] mb-2">Connected. Scans the last 30 days for receipts and order confirmations.</p>
