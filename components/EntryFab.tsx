...
const chip = (active: boolean) =>
  `text-[11px] font-medium px-3 py-2 rounded-lg cursor-pointer transition-colors ${
...
export default function EntryFab({
  ...
  return (
    <div className="fixed bottom-5 right-5 z-30" style={{ maxWidth: 'calc(100vw - 40px)' }}>
      {toast && (
        <div className="absolute bottom-[72px] right-0 bg-[var(--surface)] border border-[var(--border)]/50 text-[var(--text)] text-xs rounded-lg px-3 py-2 shadow-lg whitespace-nowrap">
          {toast}
        </div>
      )}
      ...
      <div
        className="absolute bottom-[42px] right-0 origin-bottom-right transition-all duration-200"
        style={{
          opacity: fabOpen ? 1 : 0,
      ...
      >
        <div
          ...
          style={{ borderRadius: '18px 18px 8px 18px', maxHeight: '75vh', overflowY: 'auto' }}
        >
          {view === 'hub' && (
            <div className="p-3">
              <div className="fab-grid grid gap-2.5" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
              ...
              {recurringOpen && (
                <div className="border-t border-[var(--border)]/30 mt-3 pt-3 flex flex-col gap-1.5">
                  {recurringTemplates.length === 0 && (
                    <p className="text-[11px] text-[var(--muted)]">No templates yet — add one in Settings.</p>
                  )}
                  {recurringTemplates.map((t) => (
                    <div key={t.id} className="flex justify-between items-center bg-[var(--accent)]/8 rounded-lg px-2.5 py-2">
                      <div>
                        <p className="text-[11px] font-medium text-[var(--text)]">{t.name}</p>
                        <p className="text-[10px] text-[var(--muted)]">{fmt(t.amount)} · {t.cadence}</p>
                      </div>
                      <button
                        onClick={async () => { await onLogRecurring(t); flashToast(`Logged ${t.name}`); closeAll(); }}
                        className="bg-[var(--accent)] text-[var(--bg)] text-[10px] font-medium px-2.5 py-1.5 rounded-md"
                      >
                        Log now
                      </button>
                    </div>
      ...
      <button
        ...
        className="w-14 h-14 rounded-full bg-[var(--accent)] border-2 flex items-center justify-center shadow-lg relative z-10"
        style={{ borderColor: 'var(--surface)' }}
      >
        <IconPlus size={26} className="text-[var(--bg)] transition-transform duration-200" style={{ transform: fabOpen ? 'rotate(45deg)' : 'rotate(0)' }} />
      </button>
    </div>
  );
}
...
function Tile({ icon: Icon, label, primary, locked, onClick }: { icon: any; label: string; primary?: boolean; locked?: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1" style={locked ? { opacity: 0.45 } : undefined}>
      <div
      ...
      >
        <Icon size={20} className={primary ? 'text-[var(--bg)]' : 'text-[var(--accent)]'} />
        {locked && (
          <IconLock size={12} className="absolute -bottom-1 -right-1 bg-[var(--surface)] rounded-full text-[var(--muted)]" />
        )}
      </div>
      <p className="text-[10px] text-[var(--text)]">{label}</p>
    </button>
  );
}
...
function RecurringTile({ open, onClick }: { open: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1">
      <div className="w-11 h-11 rounded-xl flex items-center justify-center relative" style={{ background: 'color-mix(in srgb, var(--accent), transparent 90%)', border: '1px solid color-mix(in srgb, var(--accent), transparent 75%)' }}>
        <IconRepeat size={20} className="text-[var(--accent)]" />
        <IconChevronsDown
          size={13}
      ...
      <p className="text-[10px] text-[var(--text)]">Recurring</p>
    </button>
  );
}
...
function BackHeader({ label, onBack }: { label: string; onBack: () => void }) {
  return (
    <div className="flex items-center gap-2 px-3 pt-3 pb-1">
      <button onClick={onBack} aria-label="Back" className="text-[var(--muted)]"><IconArrowLeft size={16} /></button>
...
function TypeView({ categories, onBack, onAdd, onDone }: { categories: string[]; onBack: () => void; onAdd: (t: NewTransaction) => Promise<void>; onDone: (msg: string) => void }) {
  ...
  const [detail, setDetail] = useState<NewTransaction>({
    amount: 0, description: '', category: categories[0] || 'Other', type: 'expense',
      ...
      {mode === 'quick' && (
        <>
          <input
            ...
            className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-3 py-2.5 text-sm text-[var(--text)] placeholder:text-[var(--muted)] mb-2"
          />
          {preview && (
            <div className="bg-[var(--bg)]/30 rounded-lg p-2.5">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-base font-semibold text-[var(--text)]">{fmt(preview.amount)}</span>
                <input value={preview.description} onChange={(e) => setPreview({ ...preview, description: e.target.value })} className="flex-1 bg-[var(--surface)] border border-[var(--border)]/40 rounded px-2 py-1 text-xs text-[var(--text)]" />
              </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <input type="date" value={preview.date} onChange={(e) => setPreview({ ...preview, date: e.target.value })} className="ml-auto bg-[var(--surface)] border border-[var(--border)]/40 rounded px-1.5 py-1 text-[10px] text-[var(--muted)]" />
              </div>
              <button onClick={confirmQuick} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium">Add transaction</button>
            </div>
          )}
        </>
      )}
      ...
      {mode === 'detailed' && (
        <form onSubmit={confirmDetailed}>
          <div className="flex gap-2 mb-2">
            <input type="number" placeholder="Amount" value={detail.amount || ''} onChange={(e) => setDetail({ ...detail, amount: parseFloat(e.target.value) || 0 })} className="w-24 bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-2 text-sm text-[var(--text)]" />
            <input placeholder="Description" value={detail.description} onChange={(e) => setDetail({ ...detail, description: e.target.value })} className="flex-1 bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-2 text-sm text-[var(--text)]" />
          </div>
          <div className="flex flex-wrap gap-1.5 mb-2">
            {categories.map((c) => (
          ...
          <input type="date" value={detail.date} onChange={(e) => setDetail({ ...detail, date: e.target.value })} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-xs text-[var(--text)] mb-2" />
          <button type="submit" className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium">Add transaction</button>
        </form>
      )}
    </div>
  );
...
function VoiceView({ categories, onBack, onAdd, onDone }: { categories: string[]; onBack: () => void; onAdd: (t: NewTransaction) => Promise<void>; onDone: (msg: string) => void }) {
  ...
  return (
    <div className="p-3">
      ...
      {unsupported && (
        <p className="text-xs text-[var(--muted)] mt-2">Voice input isn&apos;t supported in this browser — it works in Chrome and Edge, not Firefox or Safari on some platforms.</p>
      )}
      {!unsupported && !preview && (
        <div className="text-center py-3">
          ...
          >
            <IconMicrophone size={24} className="text-[var(--bg)]" />
          </button>
          <p className="text-xs text-[var(--muted)]">{listening ? (transcript || 'Listening…') : 'Tap to speak'}</p>
        </div>
      )}
      {preview && (
        <div className="bg-[var(--bg)]/30 rounded-lg p-2.5 mt-2">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-base font-semibold text-[var(--text)]">{fmt(preview.amount)}</span>
            <input value={preview.description} onChange={(e) => setPreview({ ...preview, description: e.target.value })} className="flex-1 bg-[var(--surface)] border border-[var(--border)]/40 rounded px-2 py-1 text-xs text-[var(--text)]" />
          </div>
          ...
          <button onClick={confirm} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium">Add transaction</button>
        </div>
      )}
    </div>
  );
...
function SmsView({ categories, onBack, onBulkAdd, onDone }: { categories: string[]; onBack: () => void; onBulkAdd: (items: NewTransaction[]) => Promise<void>; onDone: (msg: string) => void }) {
  ...
  function toReviewable(found: { amount: number; description: string; date: string }[]) {
    setCandidates(found.map((c) => {
      const guess = guessCategory(c.description) || { category: categories[0] || 'Other', indulgence: false };
  ...
  return (
    <div className="p-3">
      ...
      {!candidates && !csvData && (
        <>
          <textarea
            ...
            rows={4}
            className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-3 py-2 text-xs text-[var(--text)] placeholder:text-[var(--muted)] mb-2"
          />
          <button onClick={parse} disabled={!text.trim()} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium disabled:opacity-50 mb-3">Parse thread</button>
          ...
          <button onClick={() => csvFileRef.current?.click()} className="w-full flex items-center justify-center gap-2 border border-dashed border-[var(--accent)]/40 rounded-lg py-2.5 text-xs font-medium text-[var(--text)]">
            <IconFileSpreadsheet size={16} className="text-[var(--accent)]" />
            Upload a bank or UPI statement (CSV)
          </button>
          {csvError && <p className="text-[11px] text-[var(--danger)] mt-2">{csvError}</p>}
        </>
      )}

      {csvData && !candidates && (
        <div>
          <p className="text-xs text-[var(--text)] font-medium mb-1">Couldn&apos;t auto-detect columns</p>
          <p className="text-[11px] text-[var(--muted)] mb-3">Match your statement&apos;s columns once — Expensior will recognize this format going forward.</p>

          <label className="text-[10px] text-[var(--muted)] block mb-1">Date column</label>
          <select value={mapDate} onChange={(e) => setMapDate(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-xs text-[var(--text)] mb-2">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <label className="text-[10px] text-[var(--muted)] block mb-1">Description column</label>
          <select value={mapDesc} onChange={(e) => setMapDesc(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-xs text-[var(--text)] mb-2">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <label className="text-[10px] text-[var(--muted)] block mb-1">Amount column (debit or signed amount)</label>
          <select value={mapAmount} onChange={(e) => setMapAmount(parseInt(e.target.value))} className="w-full bg-[var(--bg)]/40 border border-[var(--border)]/50 rounded-lg px-2 py-1.5 text-xs text-[var(--text)] mb-3">
            {csvData.headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
          </select>

          <div className="flex gap-2">
            <button onClick={confirmMapping} className="flex-1 bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium">Continue</button>
            <button onClick={() => setCsvData(null)} className="border border-[var(--border)]/70 text-[var(--muted)] rounded-lg px-3 text-xs">Cancel</button>
          </div>
        </div>
      )}
      ...
      {candidates && (
        <>
          {candidates.length === 0 && <p className="text-xs text-[var(--muted)]">No amounts found.</p>}
          <div className="flex flex-col gap-1.5 mb-2 max-h-[240px] overflow-auto">
            {candidates.map((c, i) => (
              <label key={i} className="flex items-center gap-2 bg-[var(--bg)]/30 rounded-lg px-2.5 py-2 cursor-pointer">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-[var(--text)] truncate">{c.description}</p>
                  <p className="text-[10px] text-[var(--muted)]">{c.date} · {c.category}</p>
                </div>
                <span className="text-xs font-medium text-[var(--text)] shrink-0">{fmt(c.amount)}</span>
              </label>
            ))}
          </div>
          {candidates.length > 0 && (
            <button onClick={confirm} disabled={selectedCount === 0} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium disabled:opacity-50">
...
function ScanView({ categories, hasApiKey, onBack, onAdd, onDone }: { categories: string[]; hasApiKey: boolean; onBack: () => void; onAdd: (t: NewTransaction) => Promise<void>; onDone: (msg: string) => void }) {
    ...
    return (
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-2 mt-1">
          <IconLock size={16} className="text-[var(--muted)]" />
          <p className="text-xs font-medium text-[var(--text)]">Needs a Claude API key</p>
        </div>
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const guess = guessCategory(data.merchant) || { category: categories[0] || 'Other', indulgence: false };
    ...
    <div className="p-3">
      ...
      {!preview && (
        <div
          ...
          className="border border-dashed border-[var(--accent)]/40 rounded-lg p-5 text-center cursor-pointer mt-1"
        >
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
          {loading ? (
            <p className="text-xs text-[var(--muted)]">Reading image…</p>
          ) : (
            <>
              <IconScan size={26} className="text-[var(--accent)] mx-auto mb-1.5" />
              <p className="text-xs font-medium text-[var(--text)]">Tap to upload a photo</p>
              <p className="text-[10px] text-[var(--muted)] mt-1">Receipt, invoice, or order screenshot</p>
            </>
          )}
      ...
      {preview && (
        <div className="bg-[var(--bg)]/30 rounded-lg p-2.5 mt-1">
          <div className="flex items-center gap-2 mb-2">
            <IconReceipt size={18} className="text-[var(--accent)]" />
            <span className="text-base font-semibold text-[var(--text)]">{fmt(preview.amount)}</span>
          </div>
          <input value={preview.description} onChange={(e) => setPreview({ ...preview, description: e.target.value })} className="w-full bg-[var(--surface)] border border-[var(--border)]/40 rounded px-2 py-1.5 text-xs text-[var(--text)] mb-2" />
          <div className="flex flex-wrap gap-1.5 mb-2">
            {categories.map((c) => (
              <span key={c} className={chip(preview.category === c)} style={{ padding: '4px 10px' }} onClick={() => setPreview({ ...preview, category: c })}>{c}</span>
            ))}
...
function GmailView({
  ...
  if (!connected) {
    return (
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-2 mt-1">
          <IconMail size={18} className="text-[var(--accent)]" />
          <p className="text-xs font-medium text-[var(--text)]">Not connected yet</p>
        </div>
        ...
        <button onClick={connect} disabled={connecting} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium disabled:opacity-60">
  ...
  return (
    <div className="p-3">
      ...
      {!candidates && (
        <div className="mt-1">
          <p className="text-[11px] text-[var(--muted)] mb-2">Connected. Scans the last 30 days for receipts and order confirmations.</p>
          <button onClick={scan} disabled={scanning} className="w-full bg-[var(--accent)] text-[var(--bg)] rounded-lg py-2 text-xs font-medium disabled:opacity-60">
      ...
      {candidates && (
        <>
          {candidates.length === 0 && <p className="text-xs text-[var(--muted)]">No receipt-like emails found in the last 30 days.</p>}
          <div className="flex flex-col gap-1.5 mb-2 max-h-[240px] overflow-auto">
            {candidates.map((c, i) => (
              <label key={i} className="flex items-center gap-2 bg-[var(--bg)]/30 rounded-lg px-2.5 py-2 cursor-pointer">
                <input type="checkbox" checked={c.selected} onChange={() => setCandidates(candidates.map((x, j) => j === i ? { ...x, selected: !x.selected } : x))} />
